...
public class CreatAccntDoctor extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.userprofile); Intent intent=getIntent();
    ...
    private void findID() {
        ...
        progressDialog = new ProgressDialog(CreatAccntDoctor.this);

        String gender[]={"male","female"};
        camarapicp.setImageResource(R.drawable.save_doctor1);
    ...
    private void UpdateUser(View v) {
        ...
        if (isConnectedWifi(CreatAccntDoctor.this) || isConnectedMobile(CreatAccntDoctor.this)) {
            if (isConnected(CreatAccntDoctor.this)) {
                v.setClickable(false);
                updateUser(v);


            } else {
                new AlertBox_Inner(CreatAccntDoctor.this);
            }
        } else {
            new AlertBox_outer(CreatAccntDoctor.this);
        }
...
//        }
    ...
    private void onUdateUser(final String gname, final String gage, final String gmobile, final String gpass, String gender, View v, final String idf) {
        ...
        RestBuilderPro.getService().doctoraccntCreatupdate("doctorupdate",gname,gage,gmobile,gpass,gender,encodeImage,idf).enqueue(new retrofit2.Callback<AccntCreatItem>() {
            @Override
               ...
               progressDialog.dismiss();
                if(response.isSuccessful())
                {
                    ...
                    if(succ==1)
                    {
                        ...
                        try {
                            MainDataBAse dataBAse=new MainDataBAse(CreatAccntDoctor.this);
                            dataBAse.open();
                         ...
                         //   Log.e("email",email+" "+url+gname+gpass+idf+gmobile+gage);
                            dataBAse.updateuserTable(DATABASE_C.TABLE_NAME.T_USER, new UserDbItem(gname, email, gpass,idf,gmobile,gage, url), 1, "doctorlogin");
                            dataBAse.close();
                            DataCheckHolder.getInstance().setDistributor_id(true);
                            Intent intent = new Intent(CreatAccntDoctor.this, UserChoicePage.class);
                    ...
                    if(succ==303)
                    {
                        ...
                        new AlertBox_(error,CreatAccntDoctor.this);
                    }

                }
                else {
                    new AlertBox_("error_connecting",CreatAccntDoctor.this);
                }
            }
    ...
    private void onLoginSuccesUser(String gname, String gemail, String gage, String gmobile, String gpass, String gender, final View v) {
        ...
        RestBuilderPro.getService().doctoraccntCreat("doctorregister",gname,gemail,gage,gmobile,gpass,gender,encodeImage).enqueue(new retrofit2.Callback<AccntCreatItem>() {
            @Override
            public void onResponse(Call<AccntCreatItem> call, Response<AccntCreatItem> response) {
                v.setClickable(true);
                progressDialog.dismiss();
                if(response.isSuccessful())
                {
                    ...
                    }else if(ch==404)
                    {
                        ...
                        new AlertBox_(errormsg, CreatAccntDoctor.this);
                    }
                    else if (ch == 303) {
    ...
    private boolean valdate() {
        boolean check=true;
        String _validno=validno.getText().toString();
        String _emailvali=emailvali.getText().toString().trim();
    ...
    private void POstSend(final View v, String valid, final String email) {
        ...
        RestBuilderPro.getService().doctorkey("verifydockey",email,valid).enqueue(new retrofit2.Callback<VarificationItem>() {
            @Override
            public void onResponse(Call<VarificationItem> call, Response<VarificationItem> response) {
                ...
                if(response.isSuccessful())
                {
                    ...
                    if(ch==1) {
                        ...
                        dataBase.open();
                        try {
                            dataBase.createuserTable(DATABASE_C.TABLE_NAME.T_USER, new UserDbItem(uname,email, password, user_id,mobile,age, url),"doctorlogin");
                            dataBase.close();
                            DataCheckHolder.getInstance().setDistributor_id(true);
                            Intent intent = new Intent(CreatAccntDoctor.this, UserChoicePage.class);
                            startActivity(intent);
                    ...
                    if(ch==303){
                        ...
                        new AlertBox_(vaif.getErrormsg(),CreatAccntDoctor.this);
                    }
                    else
                    {
                        new AlertBox_("error connecting",CreatAccntDoctor.this);
                    }
                }
            }
    ...
    private void takeapic() {
        ActivityCompat.requestPermissions(CreatAccntDoctor.this,
                new String[]{
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(CreatAccntDoctor.this,android.R.layout.select_dialog_item, choice);
...
//cameratestdemo();
    ...
    private void picassaurl(String h) {
        try {
            Picasso picasso=new Picasso.Builder(CreatAccntDoctor.this).listener(new Picasso.Listener() {
                @Override
        ...
        switch (requestCode) {
            case 1: {
                ...
                if (grantResults.length > 0
                    ...
                    Toast.makeText(CreatAccntDoctor.this, "Permission denied to read your External storage", Toast.LENGTH_SHORT).show();
                }
                return;
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode==CreatAccntDoctor.RESULT_OK) {
...

            {
                ...
                try{
                    ...
                    if(op.exists()){
                        ...
                        Picasso.with(CreatAccntDoctor.this).load(op).into(camarapicp);
    ...
    public void onBackPressed() {
        ...
        if(checkf){
            ...
            Intent intent=new Intent(CreatAccntDoctor.this,LoginActivity.class);
            startActivity(intent);
            this.finish();
        }
        else
